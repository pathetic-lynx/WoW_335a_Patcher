...
static std::string native_open_dialog() {
  FILE* f = popen(
      "osascript -e 'POSIX path of (choose file with prompt \"Select "
      "WoW.exe\")' 2>/dev/null",
      "r");
  if (!f) return "";
  char buf[512] = {};
...
static bool path_is_target_file() {
  ...
  std::error_code ec;
  auto sz =
      static_cast<std::streamsize>(std::filesystem::file_size(g_path, ec));
...
static void set_path(const char* p) {
  ...
  g_statuses.clear();
  if (path_is_target_file()) action_verify();
}

static ImVec4 status_col(PatchStatus s) {
  switch (s) {
    case PatchStatus::Applied:
      return {0.20f, 0.85f, 0.35f, 1.f};
    case PatchStatus::Unpatched:
      return {1.00f, 0.78f, 0.10f, 1.f};
    case PatchStatus::Mismatch:
      return {1.00f, 0.30f, 0.30f, 1.f};
  }
...
static void action_apply() {
  ...
  for (size_t i = 0; i < g_patches.size(); i++) {
    ...
    PatchStatus st =
        (i < g_statuses.size()) ? g_statuses[i] : PatchStatus::Mismatch;
...
static void render(int win_w, int win_h) {
  ...
  if (ImGui::InputText("##path", g_path, sizeof(g_path))) g_statuses.clear();
  ...
  if (ImGui::Button("Browse...")) {
    ...
    if (!chosen.empty()) set_path(chosen.c_str());
  }

  // Hint shown only when path is empty
  if (g_path[0] == '\0')
    ImGui::TextDisabled(
        "  Tip: type a path, click Browse, or drag & drop a file here");
  ...
  if (path.empty()) {
    ...
    std::error_code ec;
    auto sz =
        static_cast<std::streamsize>(std::filesystem::file_size(path, ec));
    if (ec) {
      ...
      ImGui::TextColored({0.2f, 0.85f, 0.35f, 1.f}, "Status: OK  (%lld bytes)",
                         (long long)sz);
    }
  }
  ImGui::Spacing();
  ...
  if (ImGui::BeginTable("##ptable", 2, ImGuiTableFlags_SizingStretchProp)) {
    ...
    for (size_t i = 0; i < g_patches.size(); i++) {
      ...
      if (i < g_statuses.size())
        ImGui::TextColored(status_col(g_statuses[i]), "[%s]",
                           status_str(g_statuses[i]));
      else
        ImGui::TextDisabled("[---]");
  ...
  if (ImGui::SmallButton("All"))
    for (auto& p : g_patches) p.enabled = true;
  ImGui::SameLine();
  if (ImGui::SmallButton("None"))
    for (auto& p : g_patches) p.enabled = false;
  ImGui::EndChild();
  ...
  if (g_dragging) {
    ...
    dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.f,
                {(win_w - tsz.x * 2.f) * 0.5f, (win_h - tsz.y * 2.f) * 0.5f},
...
int main(int argc, char** argv) {
  ...
  SDL_Window* window = SDL_CreateWindow(
      "WoW 3.3.5a Patcher", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 960,
      640, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
  if (!window) {
