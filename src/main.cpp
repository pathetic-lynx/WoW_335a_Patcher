...
static std::string read_line(const char* prompt = "") {
  ...
  std::string line;
  if (!std::getline(std::cin, line)) std::cin.clear();
  ...
  for (const auto& p : g_patches) {
    ...
    switch (check_patch(path, p)) {
      case PatchStatus::Applied:
        std::cout << C::GRN << "[APPLIED  ]" << C::RST;
        break;
      case PatchStatus::Unpatched:
        std::cout << C::YEL << "[UNPATCHED]" << C::RST;
        break;
      case PatchStatus::Mismatch:
        std::cout << C::RED << "[MISMATCH ]" << C::RST;
        break;
    }
    std::cout << "\n";
  }
...
static void do_apply(const std::string& path) {
  ...
  for (size_t i = 0; i < g_patches.size(); i++)
    if (!g_patches[i].enabled) statuses[i] = check_patch(path, g_patches[i]);
  ...
  if (fail == 0)
    ...
    std::cout << C::YEL << C::BOLD << "  " << ok << " written, " << fail
              << " failed.\n"
              << C::RST;
  wait_enter();
}
...
static void do_toggle() {
  while (true) {
    ...
    std::cout << C::DIM
              << "  (number to toggle | a=all on | n=all off | b=back)\n\n"
              << C::RST;
    for (size_t i = 0; i < g_patches.size(); i++) {
      const auto& p = g_patches[i];
      std::cout << "  " << std::setw(2) << (i + 1) << ". ";
      std::cout << (p.enabled ? C::GRN : C::DIM) << (p.enabled ? "[x]" : "[ ]")
                << C::RST;
      std::cout << " " << p.name << "\n";
    }
    std::cout << "\n";
...
static void print_header(const std::string& path) {
  ...
  if (!fs::exists(path)) {
    ...
    if (ec)
      ...
      std::cout << C::YEL << "  Status: SIZE MISMATCH (" << sz << " vs "
                << kExpectedFileSize << ")\n"
                << C::RST;
    else
  ...
  std::cout << C::DIM << "  Patches selected: " << enabled << "/"
            << g_patches.size() << "\n\n"
            << C::RST;
}
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    if (choice == "0")
      ...
      do_verify(path);
    else if (choice == "2" && fs::exists(path))
      do_apply(path);
    else if (choice == "2") {
      std::cout << C::RED << "  File not found.\n" << C::RST;
      wait_enter();
    } else if (choice == "3")
      do_toggle();
  }
